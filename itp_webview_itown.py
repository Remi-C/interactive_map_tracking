...
class ITP_WebView_iTowns(QObject):
    ...
    def moveMap_QGIS(self):
        ...
        #
        point_itowns = QgsPoint(self.e, self.n)
        point_qgis = xform.transform(point_itowns)
        #
        if self.synch_QGIS_iTowns:
            self.synch_QGIS_iTowns_finish = False
            #
            mapCanvas.setCenter(point_qgis)
            qgis_mapcanvas_tools.refreshMapCanvas(self.iface)
        #
        qgis_log_tools.logMessageINFO("point_qgis: " + str(point_qgis.x()) + ", " + str(point_qgis.y()))
